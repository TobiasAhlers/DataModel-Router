...
def test_set_field_value_valid(client: TestClient):
    ...
    """
    response = client.put("testdatamodel/1/name", params={"name": "Bob"})
...
def test_set_field_value_invalid(client: TestClient):
    ...
    """
    response = client.put(
        "testdatamodel/1/invalid_field", params={"invalid_field": "value"}
    )
...
def test_no_entry(client: TestClient):
    ...
    """
    response = client.put("testdatamodel/2/name", params={"name": "Bob"})
    assert response.status_code == 404
...
def test_multiple_entries(client: TestClient):
    ...
    """
    response = client.put("testdatamodel/1/age", params={"age": 35})
    assert response.status_code == 200
    assert response.json() == {"id": 1, "name": "Alice", "age": 35}

    TestDataModel(name="Bob", age=35).save()
    response = client.put("testdatamodel/1/age", params={"age": 30})
    assert response.status_code == 200
    assert response.json() == {"id": 1, "name": "Alice", "age": 30}
    response = client.put("testdatamodel/2/age", params={"age": 40})
    assert response.status_code == 200
...
def test_multiple_models(client: TestClient):
    ...
    response = client.put("testdatamodel2/1/city", params={"city": "Los Angeles"})
    assert response.status_code == 200
    assert response.json() == {"id": 1, "country": "USA", "city": "Los Angeles"}
