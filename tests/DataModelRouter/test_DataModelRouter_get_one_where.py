...
def test_get_all_where_invalid(client: TestClient):
    ...
    """
    response = client.get("testdatamodel/get_one", params={"invalid_param": "value"})
...
def test_no_entry(client: TestClient):
    ...
    """
    response = client.get("testdatamodel/get_one", params={"id": 2})
...
def test_multiple_models(client: TestClient):
    ...
    """
    response = client.get("testdatamodel/get_one", params={"age": 30})
    assert response.status_code == 200
    assert response.json() == {"id": 1, "name": "Alice", "age": 30}

    response = client.get("testdatamodel2/get_one", params={"city": "New York"})
    assert response.status_code == 200
